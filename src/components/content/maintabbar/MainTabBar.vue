<template>
	<tab-bar>
		<tab-bar-item path="/home">
			<img src="~assets/img/tabbar/home.png" slot="item-icon">
			<img src="~assets/img/tabbar/home-active.png" slot="item-icon-active">
			<div slot="item-text">首页</div>
		</tab-bar-item>
		<tab-bar-item path="/categories">
			<img src="~assets/img/tabbar/categories.png" slot="item-icon">
			<img src="~assets/img/tabbar/categories-active.png" slot="item-icon-active">
			<div slot="item-text">分类</div>
		</tab-bar-item>
		<tab-bar-item path="/car">
			<img src="~assets/img/tabbar/car.png" slot="item-icon">
			<img src="~assets/img/tabbar/car-active.png" slot="item-icon-active">
			<div slot="item-text">购物车</div>
		</tab-bar-item>
		<tab-bar-item path="/profile">
			<img src="~assets/img/tabbar/profile.png" slot="item-icon">
			<img src="~assets/img/tabbar/profile-active.png" slot="item-icon-active">
			<div slot="item-text">我的</div>
		</tab-bar-item>
	</tab-bar>
</template>

<script>
	import TabBar from 'components/common/tabbar/TabBar.vue'
	import TabBarItem from 'components/common/tabbar/TabBarItem.vue'
	export default {
		components: {
			TabBar,
			TabBarItem
		}
	}
</script>

<style>
</style>
